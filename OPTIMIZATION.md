...
按「投入产出比」排序，适合作为项目演进路线图。

| 步骤 | 改进 | 耗时 | 简历含金量 | 预期 MAP 提升 |
|------|------|------|-----------|-------------|
| 1 | **Swing 召回 + ALS 矩阵分解** — 替代当前 ItemCF，Swing 对噪声更鲁棒，ALS 可生成 embedding 供后续深度模型使用 | ~30min | ⭐⭐⭐⭐ | +1~3% |
| 2 | **特征工程扩展到 30 维以上** — 加入价格分位数、类目偏好交叉、季节/新品标志、用户活跃度分桶、商品描述 embedding | ~1h | ⭐⭐⭐ | +2~5% |
| 3 | **MMR 多样性重排** — 在精排 Top-12 后做 Maximal Marginal Relevance 重排，避免推荐结果全是同款不同色 | ~20min | ⭐⭐⭐ | 体验提升 (MAP 可能不变或微降，但推荐列表更有用) |
| 4 | **采样集上跑 DeepFM** — 在 5 万用户采样集上用 PyTorch 实现 DeepFM，验证深度模型在召回+排序上的效果 | ~2h | ⭐⭐⭐⭐⭐ | 架构验证，为后续升级打基础 |
| 5 | **多路召回融合权重自动学习** — 用 LR 或简单 Ranker 学习各召回路的权重，替代手动调参 | ~1h | ⭐⭐⭐ | +1~2% |
